...
func TestMigrateSchemaV8AllowsReusingArchivedLogicalModelCode(t *testing.T) {
	...
	if err := migrateLogicalModelActiveCode(db); err != nil {
		t.Fatal(err)
	}
...
func TestMigrateSchemaAppliesUpstreamBlockToStoryOnlyDatabases(t *testing.T) {
	...
	// 模拟只有 Story 迁移（v1-v12）记录的旧分支数据库：升级后上游部分（v13-v19）按新编号补齐。
	for _, item := range schemaMigrations[:12] {
		if err := db.Create(&schemaMigration{Version: item.version, Name: item.name, Checksum: item.checksum, AppliedAt: time.Now().UTC()}).Error; err != nil {
			t.Fatal(err)
		}
	}
	...
	if err := MigrateSchema(db); err != nil {
		t.Fatalf("apply upstream migrations after story block: %v", err)
	}
	if !db.Migrator().HasColumn(&model.Resource{}, "playback_status") || !db.Migrator().HasColumn(&model.Resource{}, "playback_object_key") || !db.Migrator().HasColumn(&model.Resource{}, "playback_error") {
		t.Fatal("upgraded database did not receive resource playback columns")
	}
	status, err := ReadSchemaStatus(db)
	if err != nil {
		t.Fatal(err)
	...
	if !status.Ready || status.Current != CurrentSchemaVersion {
		t.Fatalf("unexpected upgraded schema status: %#v", status)
	}
	var applied schemaMigration
	if err := db.First(&applied, "version = ?", 16).Error; err != nil {
		t.Fatal(err)
	}
	if applied.Name != "asset_library_folders" || applied.Checksum != assetLibraryFoldersChecksum {
		t.Fatalf("migration 16 must be asset_library_folders: %#v", applied)
	}
	...
	if playback.Name != "resource_playback_variant" || playback.Checksum != resourcePlaybackChecksum {
		t.Fatalf("migration 15 must supply playback schema: %#v", playback)
	}
}
