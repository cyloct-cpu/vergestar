...
func RegisterCanvasAPI(api *gin.RouterGroup, svc *service.Service) {
	...
	RegisterComfyBridgeRoutes(api, svc)
	RegisterRunningHubRoutes(api, svc)
	RegisterSessionRoutes(api, svc)
	...
	RegisterNovelAgentRoutes(api, svc)
	RegisterNovelAgentJobStream(api, svc)
	RegisterStoryRoutes(api, svc)
