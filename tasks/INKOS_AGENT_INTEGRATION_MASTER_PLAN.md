...
## 2026-09-12 UI/交互对比实机操作：章节阅读体验对齐 InkOS（修复正文平铺）

- **实机对比**（InkOS 书籍工作台 vs 影策项目工作台）：影策把整章正文直接平铺在写作面板里（数百字撑爆页面），且阅读/编辑入口只有"阅读与编辑"一个切换——与 InkOS 的章节阅读体验差距最大的交互点。
- **修复**：
  1. 正文预览默认折叠（600 字截断 + "…"），提供"展开全文"；
  2. 新增"全屏阅读"阅读器 Modal（InkOS ChapterReader 式）：衬线字体、正文 62vh 滚动、标题带章号/字数/版本号，底部提供版本切换按钮（restore 幂等）与"编辑本章""审稿与修订"直达入口；
  3. 阅读器与折叠预览共用 chapterContent，无重复请求。
- **实机验收**：全屏阅读器打开（Modal 标题"第 1 章·快三分钟之外 2984 字·版本 1"），正文完整渲染；关闭后主区折叠预览生效；版本按钮可见。折叠+"展开全文"两态切换正常。
- 同批实机操作记录：InkOS 首页书籍卡片（新建书籍/写下一章/统计快捷）与书籍工作台聊天流结构已再次核对，影策的快捷动作 chips/真相面板/确认卡/任务卡结构与其对齐（差异项已在前几轮补齐）。


## 2026-09-12 UI 对齐迭代第二批：角色卡展开/正史统计/章节点击进阅读器

- **角色卡交互对齐 InkOS**：工作台与真相面板的角色行升级为可展开卡片（details 交互）——点击角色名展开 InkOS 角色档案 Markdown（核心标签/反差细节等，截断 1200 字），主要/次要徽标保留。实机验收：贺伯配角卡展开渲染角色档案内容。
- **正史统计**：头部统计行新增 "N 章已入正史"（completed 状态计数），与 InkOS 的 approved 语义对齐。实机验收：显示 "1 章已入正史"。
- **章节行点击直达阅读器**：右栏章节列表点击即选中并打开全屏阅读器（有正文的章节），与 InkOS 点击章节进阅读器的路径一致。
- 提交：以上随本节一并入库推送。


## 2026-09-12 分镜字段回填实机验收（部分命中，机制正确）

- 轮询旧分镜 Job（3560804c）触发回填：24 个既有镜头中可对上行（shot-002/004/006 等）已回填景别与对白（如 size=特写 + dlg=字幕：晚上八点十分，B区夜班开始）；对不上的行（镜头 ID 乱序映射）保持空——幂等语义正确：只填空字段，不覆盖。
...
## 2026-09-12 agnes-3.0-flash 稳定通道确认 + 五模式验收状态更新

- **渠道探测结论**：agnes-2.0-flash 429 限流、agnes-2.5-flash 超时、agnes-2.5-pro 403 余额耗尽、agnes-3.0-flash 200 稳定可用且输出质量好。渠道 2（easy-dotnet）仍全模型 400、渠道 5（metaso）路径不支持。agnes-3.0-flash 是当前唯一可用的稳定文本模型。
- **模型切换**：影策全局文本模型已切换为 agnes-3.0-flash。
