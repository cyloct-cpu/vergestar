...
function buildBookPrompt(bookId: string, isZh: boolean, confirmedWriteNext = false): string {
  const writeBoundary = confirmedWriteNext
    ? isZh
      ? "- 本轮已通过 Vergestar 确认执行 write_next。现在只调用 sub_agent(agent=writer) 写下一章；不要再次调用 propose_action，不要改用 architect、auditor 或 reviser。"
      : "- This turn is already confirmed by Vergestar as write_next. Call only sub_agent(agent=writer) now; do not call propose_action again or switch to architect, auditor, or reviser."
    : isZh
      ? "- 继续写、写下一章或新增章节时，先调用 propose_action，action=write_next，生成结构化确认卡；确认后才能调用 sub_agent(agent=writer)。不要直接调用 writer，也不要在聊天里输出正文代替确认卡。"
      : "- When the user asks to continue, write the next chapter, or add new chapters, first call propose_action with action=write_next to create a structured confirmation card; only after confirmation may you call sub_agent(agent=writer). Do not call writer directly or substitute chat prose for the confirmation card.";
  return isZh
...
${writeBoundary}
- 当用户要求修复章节状态、同步真相文件、重新结算伏笔但明确保留正文时，先调用 propose_action，action=repair_state，生成结构化确认卡；确认后才能调用 resync_chapter_state。不得调用 writer/reviser 改写正文。
...
${writeBoundary}
- When the user asks to repair chapter state, resynchronize truth files, or settle hooks while preserving the prose, first call propose_action with action=repair_state; only after confirmation may you call resync_chapter_state. Do not rewrite the prose with writer or reviser.
...
export function buildAgentSystemPrompt(
  ...
  if (sessionKind === "book" && bookId) return withSkills(buildBookPrompt(bookId, isZh, isConfirmedAction(options, "write_next")));
