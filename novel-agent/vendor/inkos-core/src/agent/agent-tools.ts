...
const ProposeActionParams = Type.Object({
  action: Type.Union([
    ...
    Type.Literal("write_next"),
    Type.Literal("repair_state"),
  ...
  writeNext: Type.Optional(Type.Object({
    chapterCount: Type.Optional(Type.Number({
      minimum: 1,
      maximum: 20,
      description: "Number of consecutive chapters to write after confirmation. Defaults to 1.",
    })),
  }, { description: "Structured execution args for action=write_next." })),
  repairState: Type.Optional(Type.Object({
    chapterNumber: Type.Optional(Type.Number({ minimum: 1, description: "Latest chapter number whose derived state should be repaired without changing prose." })),
  }, { description: "Structured execution args for action=repair_state." })),
...
function proposedActionSessionKind(action: ProposeActionParamsType["action"]): "book-create" | "book" | "short" | "play" | "script" | "storyboard" | "interactive-film" | "interactive-film-authoring" | "chat" {
  if (action === "create_book") return "book-create";
  if (action === "write_next") return "book";
  if (action === "repair_state") return "book";
...
function proposedActionFallbackTitle(action: ProposeActionParamsType["action"], isZh: boolean): string {
  switch (action) {
    ...
    case "write_next":
      return isZh ? "写下一章" : "Write the next chapter";
    case "repair_state":
      return isZh ? "修复章节状态" : "Repair chapter state";
    case "short_run":
...
function proposedActionPayload(
  ...
  if (params.action === "write_next") {
    const writeNext = compactObject(params.writeNext) ?? {};
    payload.writeNext = { chapterCount: writeNext.chapterCount ?? 1 };
  }
  if (params.action === "repair_state") {
    const repairState = compactObject(params.repairState);
    if (repairState) payload.repairState = repairState;
  }
...
function assertExecutableProposedAction(params: ProposeActionParamsType, payload: ActionPayload | undefined): void {
  ...
  if (params.action === "write_next") {
    const chapterCount = payload?.writeNext?.chapterCount;
    if (chapterCount !== undefined && (!Number.isInteger(chapterCount) || chapterCount < 1 || chapterCount > 20)) {
      throw new Error("writeNext.chapterCount must be an integer from 1 to 20.");
    }
    return;
  }
  if (params.action === "repair_state") {
    const chapterNumber = payload?.repairState?.chapterNumber;
    if (chapterNumber !== undefined && (!Number.isInteger(chapterNumber) || chapterNumber < 1)) {
      throw new Error("repairState.chapterNumber must be a positive integer.");
    }
    return;
  }
  if (params.action === "play_start") {
...
const WriterSubAgentParams = Type.Object({
  agent: Type.Literal("writer"),
  instruction: Type.String({ description: "Confirmed self-contained instruction for writing the next chapter." }),
  bookId: Type.Optional(Type.String({ description: "Optional book ID; defaults to the active book." })),
  chapterCount: Type.Optional(Type.Integer({ minimum: 1, maximum: 20, description: "Number of consecutive chapters to write. Defaults to the confirmed action count or 1." })),
  chapterWordCount: Type.Optional(Type.Number({ description: "Optional per-chapter length in the book's native unit." })),
});
...
export function createSubAgentTool(
  ...
  return {
    ...
    label: "Sub-Agent",
    parameters: options.architectCreateOnly
      ? ArchitectCreateSubAgentParams
      : options.writerOnly
        ? WriterSubAgentParams
        : SubAgentParams,
    ...
    async execute(
      ...
      return runWithAgentTrajectoryRole("subagent", async () => {
        ...
        try {
          ...
          if (options.writerOnly && agent !== "writer") {
            throw new Error("This confirmed write-next turn can only run the writer.");
          }
          ...
          case "writer": {
            ...
            const confirmedChapterCount = options.actionPayload?.writeNext?.chapterCount;
            const requestedCount = chapterCount ?? confirmedChapterCount ?? 1;
            if (requestedCount > 1) {
