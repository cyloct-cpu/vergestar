...
function createModeTools(params: CreateAgentToolsForModeParams) {
  ...
  if (params.sessionKind === "book" && isConfirmed("write_next")) {
    return [createSubAgentTool(params.pipeline, params.bookId, params.projectRoot, {
      actionPayload: params.actionPayload,
      language: lang,
      writerOnly: true,
  ...
  if (params.sessionKind === "book" && isConfirmed("repair_state")) {
    return [createResyncChapterStateTool(params.pipeline, params.bookId, {
      language: lang,
      defaultSkills: params.productionSkills?.("longWriting"),
      activeSkills: params.activeSkills,
    })];
  ...
  const bookTools = [
    subAgentTool,
    // Existing-book conversations must be able to propose a production
    // action before the host opens the mutating tools. The host additionally
    // filters PRODUCTION_MUTATION_TOOL_NAMES for unconfirmed turns.
    proposalTool,
