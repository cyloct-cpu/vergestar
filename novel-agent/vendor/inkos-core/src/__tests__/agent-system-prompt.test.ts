...
describe("buildAgentSystemPrompt", () => {
  ...
  describe("book mode", () => {
    ...
    it("requires a write-next confirmation proposal before chapter production", () => {
      const prompt = buildAgentSystemPrompt("my-book", "zh", "book");
      expect(prompt).toContain("继续写、写下一章或新增章节时，先调用 propose_action");
      expect(prompt).toContain("action=write_next");
      expect(prompt).toContain("确认后才能调用 sub_agent(agent=writer)");
    });
