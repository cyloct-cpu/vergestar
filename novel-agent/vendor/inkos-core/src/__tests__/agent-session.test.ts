  ...
  const streamSimple = vi.fn((model: any, context: any, options: any) => {
    ...
    const message = last?.role === "toolResult"
      ...
      : prompt === "model error"
        ...
        : prompt === "propose write next"
        ? assistant([
            {
              type: "toolCall",
              id: "proposal-write-next-1",
              name: "propose_action",
              arguments: {
                action: "write_next",
                title: "写下一章",
                summary: "确认后运行 Writer 生产管线。",
                instruction: "继续写当前书的下一章。",
                writeNext: { chapterCount: 1 },
              },
            },
          ], timestamp)
...
describe("runAgentSession cache — bookId switch", () => {
  ...
  it("gates existing-book writing behind write_next and exposes only writer after confirmation", async () => {
    const model = { provider: "x", id: "y", api: "anthropic-messages" } as any;
    const pipeline = {
      runWithAgentContext: vi.fn(async (_context: unknown, task: () => Promise<unknown>) => task()),
      writeNextChapter: vi.fn(async () => ({ chapterNumber: 1, wordCount: 1200, status: "ready-for-review" })),
    } as any;

    await runAgentSession(
      {
        sessionId: "book-write-proposal-session",
        bookId: "book-a",
        sessionKind: "book",
        language: "zh",
        pipeline,
    ...
    const unconfirmedTools = agentInstances.at(-1).state.tools.map((tool: any) => tool.name);
    expect(unconfirmedTools).toContain("propose_action");
    expect(unconfirmedTools).not.toContain("sub_agent");

    await runAgentSession(
      {
        sessionId: "book-write-confirmed-session",
        bookId: "book-a",
        sessionKind: "book",
        actionSource: "button",
    ...
    expect(agentInstances.at(-1).state.tools.map((tool: any) => tool.name)).toEqual(["sub_agent"]);
    expect(JSON.stringify(agentInstances.at(-1).state.tools[0].parameters)).toContain('"const":"writer"');
    expect(JSON.stringify(agentInstances.at(-1).state.tools[0].parameters)).not.toContain('"const":"auditor"');
    expect(JSON.stringify(agentInstances.at(-1).state.tools[0].parameters)).not.toContain('"const":"reviser"');
  });
