  ...
  async createOutline(input: ShortFictionOutlineInput): Promise<ShortFictionOutline> {
    const response = await retryShortFictionCall(() =>
      completeLongForm({
        ...
        generate: (continuationMessages) => this.chat(continuationMessages, { temperature: 0.55, maxTokens: 16_384 }),
        onContinuation: (pass) => this.log?.warn(`[${this.name}] Output limit reached; continuing pass ${pass}.`),
      }), this.name, this.log);
...
export class ShortFictionOutlineReviewerAgent extends BaseAgent {
  ...
  async reviewOutline(input: ShortFictionOutlineReviewInput): Promise<string> {
    const response = await retryShortFictionCall(() =>
      ...
      ], { temperature: 0.3, maxTokens: 16_384 }), this.name, this.log);
